...
export default function LoginPage({ onAuth, gotoRegister, t, lang }) {
      ...
      <div className="w-full max-w-sm bg-[#111827] rounded-2xl border border-gray-800 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? t("auth_loading") : t("auth_login_btn")}
          </button>
        </form>
        <p className="text-center text-xs text-gray-500 mt-4">
          {t("auth_no_account")}{" "}
          <button onClick={gotoRegister} className="text-blue-400 hover:text-blue-300 underline">{t("auth_register_link")}</button>
        </p>
      </div>
    </div>
  );
